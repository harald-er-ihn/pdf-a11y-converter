...
aggressives Paragraph-Merging (jetzt mit Same-Line Detection).
"""
...
class DOMTransformer:
    ...
    def _merge_paragraphs(
        ...
        for nxt in elements[1:]:
            if curr.type == "p" and nxt.type == "p":
                ...
                # ARCHITEKTUR-FIX: Horizontale Elemente auf derselben Linie mergen
                v_overlap = max(
                    0.0,
                    min(curr.bbox[3], nxt.bbox[3])
                    - max(curr.bbox[1], nxt.bbox[1]),
                )
                is_same_line = v_overlap > (h_curr * 0.2)

                if is_same_line or (
                    x_align and -10.0 <= v_gap <= (h_curr * 1.2)
                ):
    ...
    @classmethod
    def merge_forms(
        cls, dom: SpatialDOM, forms: List[SpatialElement]
    ) -> SpatialDOM:
    ...
    def merge_formulas(cls, dom: SpatialDOM, formula_md: str) -> SpatialDOM:
        ...
        latex_formulas = [
            m.group(2).strip()
            for m in re.finditer(regex, formula_md, flags=re.DOTALL)
        ]
