...
Robuste Filter gegen Uhrzeiten und Datumsangaben in reinen Daten-Matrizen.
"""
...
def extract_footnotes_local(pdf_path: Path) -> Dict[str, Any]:
    """Sucht lokal nach Fußnoten und ignoriert strikt Datumsformate."""
        ...
        with fitz.open(pdf_path) as doc:
            for page_num, page in enumerate(doc, start=1):
                ...
                for block in blocks:
                    ...
                    # FIX: Ignoriere reine Daten und Uhrzeiten in Datenblättern!
                    if re.match(r"^\d{1,2}\.\d{1,2}\.\d{2,4}", block_text):
                        continue
                    if re.match(r"^\d{1,2}[:\.]\d{2}\b", block_text):
                        continue

                    # FIX: Ignoriere einfache Seitenzahlen (verhindert Phantom-Notizen)
                    if re.match(
                        r"^(page|seite)?\s*\d+\s*$", block_text.lower()
                    ):
                        continue
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fußnoten Experte (Local/PyMuPDF)"
    )
