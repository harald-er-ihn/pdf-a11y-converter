...
class CoordinateAdapter:
    ...
    @classmethod
    def pixel_to_points(
        cls, bbox: List[float], dpi: float = 144.0
    ) -> List[float]:
    ...
    def convert_to_pdf_points(
        ...
        if system == "top_left_points":
            ...
            logger.error(
                "❌ Kritisches Koordinaten-Mismatch: System '%s' unbekannt.",
                system,
            )
            raise ValueError(
                f"Unsupported coordinate system declared: {system}"
            )

    @classmethod
    def normalize_dom(
        cls, dom: SpatialDOM, system: str, dpi: float = 144.0
    ) -> None:
        ...
        for page in dom.pages:
            for el in page.elements:
                el.bbox = cls.convert_to_pdf_points(
                    el.bbox, system, page.height, dpi
                )
    ...
    def normalize_elements(
        ...
        for el in elements:
            el.bbox = cls.convert_to_pdf_points(
                el.bbox, system, page_height, dpi
            )
