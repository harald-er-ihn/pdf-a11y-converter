...
class App(CustomTkDnD):
    ...
    def _populate_tab(
        self, tab: ctk.CTkFrame, path: str, fallback: str
    ) -> None:
    ...
    def open_about_window(self) -> None:
        ...
        self._populate_tab(
            tabview.add("Lizenzen"),
            "static/docs/licenses.txt",
            "Lizenzen fehlen.",
        )
    ...
    def start_conversion(self) -> None:
        ...
        threading.Thread(
            target=self._run_phase_1, args=(self.selected_file,)
        ).start()
    ...
    def _run_phase_1(self, input_path: str) -> None:
        ...
        logger.info(
            "🔍 Prüfe Original-PDF (%s)...", os.path.basename(input_path)
        )
    ...
    def _run_phase_3(
        ...
        try:
            ...
            if success:
                ...
                logging.getLogger("pdf-converter").info(
    ...
    def _finish(self, success: bool, output_path: str) -> None:
        ...
        if success:
            self.status_label.configure(
                text=f"Erfolgreich: {os.path.basename(output_path)}",
                text_color="green",
            )
        else:
    ...
    def show_visual_screenreader(self) -> None:
        ...
        target_pdf = None
        if self.converted_output_path and os.path.exists(
            self.converted_output_path
        ):
        ...
        threading.Thread(
            target=self._run_vsr_thread, args=(target_pdf,)
        ).start()
    ...
    def _run_vsr_thread(self, pdf_path: str) -> None:
        ...
        try:
            ...
            logger.info(
                "📄 Analysiere physischen Tag-Baum von %s...", p_path.name
            )
            ...
            if success:
                self.after(
                    0, lambda: webbrowser.open(f"file://{vsr_html.absolute()}")
                )
                logger.info(
                    "👁️ Visual Screenreader geöffnet: %s", vsr_html.name
                )
            else:
                msg = (
                ...
                self.after(
                    0, lambda: messagebox.showwarning("Keine Tags", msg)
                )
