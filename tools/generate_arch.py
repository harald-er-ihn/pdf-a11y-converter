...
def get_dynamic_workers() -> list[dict]:
    ...
    for d in workers_dir.iterdir():
        ...
        if (
            ...
            display_name = " ".join(
                p.capitalize() for p in clean_name.split("_")
            )
...
def generate_architecture_graph() -> None:
    ...
    # Execution Flow
    dot.edge(
        "Orchestrator", "PluginManager", label=" Map/Reduce", fontsize="9"
    )
    ...
    print(
        f"✅ Architektur-Graph (Clean Architecture) generiert: {out_path}.svg"
    )
