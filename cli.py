...
Schreibt einen sauberen Audit-Trail für Systemadministratoren und fängt
Anwendungsfehler (wie inkompatible Formulare) sauber ab.
"""
...
def main() -> None:
    ...
    if args.usage:
        print(
            "  python cli.py <eingabe.pdf> [-o <ausgabe.pdf>] [--visualscreenreader]"
        )
        sys.exit(0)
    ...
    if initial_check.get("passed", False):
        ...
        logger.info(
            "🔴 Original-PDF ist NICHT barrierefrei. Starte Rekonstruktion..."
        )
    ...
    try:
        ...
        generate_pdf_from_spatial(
            sp_dom, args.input, imgs, str(out_p), docinfo, lang
        )
        ...
        # ARCHITEKTUR-FIX: Sauberes, mehrzeiliges Ausgeben der Custom Error Messages
        logger.error("❌ System-Abbruch:\n%s", ve)
        sys.exit(1)
    except Exception as e:  # pylint: disable=broad-exception-caught
        logger.error("❌ Kritischer Systemfehler: %s", e)
        sys.exit(1)
