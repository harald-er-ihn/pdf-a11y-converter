...
def main() -> None:
    ...
    logger.info("🤖 Lade NLLB-200 Translation-Experten lokal aus: %s", LOCAL_MODEL_DIR.name)
    ...
    try:
        ...
        tokenizer = AutoTokenizer.from_pretrained(str(LOCAL_MODEL_DIR), src_lang="eng_Latn", local_files_only=True)
        model = AutoModelForSeq2SeqLM.from_pretrained(str(LOCAL_MODEL_DIR), local_files_only=True).to(device)
        ...
        for key, text in texts_to_translate.items():
            ...
            with torch.no_grad():
                outputs = model.generate(**inputs, forced_bos_token_id=target_id, max_length=100)
