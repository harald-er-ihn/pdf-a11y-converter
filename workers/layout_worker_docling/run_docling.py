...
def _setup_docling(force_ocr: bool) -> Any:
    ...
    if force_ocr:
        logger.info(
            "⚠️ FORCE-OCR: Ignoriere PDF-Textlayer und erzwinge Visual OCR."
        )
        try:
...
def _add_picture_elements(
    doc: Any,
    page_heights: Dict[int, float],
    spatial_dom: Dict[str, Any],
) -> None:
    """Ergänzt extrahierte Docling-Bilder als echte Figure-Elemente im DOM."""
    for idx, picture in enumerate(getattr(doc, "pictures", []) or []):
        try:
            ...
            logger.debug("Picture-Element übersprungen: %s", e)
...
def extract_spatial_data_docling(
    ...
    _add_picture_elements(doc, page_heights, spatial_dom)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Layout Worker (Primary/Docling)"
    )
    parser.add_argument("--input", required=True, help="Pfad zum Eingabe-PDF")
    parser.add_argument(
        "--output", required=True, help="Pfad zur Ausgabe-JSON"
    )
    parser.add_argument(
        "--force-ocr", action="store_true", help="Erzwingt OCR"
    )
