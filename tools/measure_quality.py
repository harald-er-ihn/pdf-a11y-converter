...
"""
Quality Measurement Tool (Oracle Validator) - Version 4.2 (Strict F1 Score).
Nutzt eine ultra-robuste NLP Metrik (Bag-of-Words F1 Score).
Behebt Parser-Statusverluste (innere HTML-Tags) durch dynamische Auswertung.
ARCHITEKTUR FIX: Verhindert False-Positive 100% Scores bei verschmolzenen
Paragraphen durch F1-Penalty für Untersegmentierung.
"""
...
class VSRTreeParser(HTMLParser):
    ...
    def handle_data(self, data: str) -> None:
        ...
        if in_label and label_node and label_node.get("parent"):
            label_node["parent"]["semantic_label"] = data.upper()
        elif in_text:
...
def parse_markdown_blocks(md_text: str) -> List[SemanticBlock]:
    """Test-Oracle Parser: Simuliert die semantische Erwartung."""
    blocks: List[SemanticBlock] = []
    current_tag = "P"
    ...
    for line in md_text.splitlines():
        ...
        if line.startswith("$$"):
            _push()
            blocks.append(
                SemanticBlock("FORMULA", line.replace("$$", "").strip())
            )
            continue
...
def get_bow_f1_score(md_text: str, vsr_text: str) -> float:
    """Berechnet robuste NLP Inclusion via Bag-of-Words F1 Score."""
    md_words = re.sub(r"\W+", " ", md_text.lower()).split()
    vsr_words = re.sub(r"\W+", " ", vsr_text.lower()).split()

    if not md_words and not vsr_words:
        return 1.0
    if not md_words or not vsr_words:
        return 0.0
    ...
    # ARCHITEKTUR-FIX: F1 Score bestraft verschmolzene Riesen-Absätze!
    precision = matched / len(vsr_words)
    recall = matched / len(md_words)
...
def evaluate_document(
    ...
    for md_b in md_blocks:
        ...
        for v_idx in range(len(vsr_blocks)):
            for window_size in range(1, 5):
                ...
                score = get_bow_f1_score(md_b.text, combined_text)
    ...
    content_match = (
        (sum(content_scores) / len(md_blocks)) if md_blocks else 100.0
    )
...
<head>
    <meta charset="UTF-8">
    <title>PDF A11y Converter - Quality Report V4.2</title>
    <style>
...
<body>
    <h1>📊 PDF A11y - Quality Audit V4.2 (Strict F1 Score)</h1>
    <div class="dashboard">
        <div class="score-box">
...
def main() -> None:
    ...
    logger.info("🔍 Analysiere Testdaten mit F1 Metric V4.2...")
    results = []

    for md_file in tests_dir.glob("*.md"):
        if md_file.name in ["Test_08_Forms.md", "Test_09_Formulas.md"]:
            logger.info("⏭️ Überspringe %s", md_file.name)
            continue
        ...
        if not vsr_file.exists():
            logger.warning("⚠️ Kein VSR-Output für %s.", md_file.name)
            results.append(
                {
                    "name": md_file.name,
                    "content": 0.0,
    ...
    logger.info("🎉 Qualitätsmessung V4.2 abgeschlossen!")
