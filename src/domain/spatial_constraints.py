...
class SpatialConstraintSolver:
    ...
    @classmethod
    def subtract_text_region(
        cls, base_text: str, sub_text: str
    ) -> Tuple[str, str]:
        """
        ...
        if not blocks:
            ...
            if idx != -1:
                return base_norm[:idx].strip(), base_norm[
                    idx + len(sub_norm) :
                ].strip()
            return base_norm, ""
    ...
    def insert_element_at_position(
        ...
        """
        before_text, after_text = cls.subtract_text_region(
            base_el.text or "", sub_text
        )
        ...
        y_mid_top = min(
            max(base_el.bbox[1], insert_el.bbox[1]), base_el.bbox[3]
        )
        y_mid_bottom = min(
            max(base_el.bbox[1], insert_el.bbox[3]), base_el.bbox[3]
        )
        ...
        if before_text:
            ...
            result.append(
                SpatialElement(
                    type=base_el.type, text=before_text, bbox=bbox_before
                )
            )
        ...
        if after_text:
            ...
            result.append(
                SpatialElement(
                    type=base_el.type, text=after_text, bbox=bbox_after
                )
            )
