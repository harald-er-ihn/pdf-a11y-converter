...
from src.table_repair_facade import TableRepairFacade
...
def _get_mathml(latex_text: str) -> str:
    """Versucht Formeln in Screenreader-freundliches MathML zu konvertieren."""
    try:
...
def _build_element_html(el: SpatialElement) -> str:
    ...
    if tag == "table":
        ...
        # Architektonischer Fix: Delegation an Facade Pattern
        html_t = TableRepairFacade.repair_html_table(html_t)
    ...
    if tag in ["list", "ul", "ol"]:
        ...
        for item in el.items or []:
            ...
            i_txt = _auto_linkify(
                html.escape(remove_control_characters(raw_i))
            )
    ...
    if tag == "figure":
        ...
        if el.items:
            ...
            cap_txt = html.escape(remove_control_characters(raw_cap))
            fig_html += (
                f'<pac-caption style="{i_style}">{cap_txt}</pac-caption>'
            )
    ...
    if tag == "form":
        ...
        txt_html = html.escape(
            remove_control_characters(el.text or "Formularfeld")
        )
        return (
            f"{wrapper_start}"
    ...
    if tag == "formula":
        ...
        mathml_attr = (
            f' data-mathml="{html.escape(mathml, quote=True)}"'
            if mathml
            else ""
        )
        return (
